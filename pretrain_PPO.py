...
parser.add_argument('--ppo_epochs', type=int, default=10000, help='Proximal Policy Optimization epochs')
parser.add_argument('--clip_eps', type=float, default=0.2, help='Clipping epsilon parameter for PPO')
...
def evaluate(batch_obs, inputs_map, batch_acts, targets):
    probs = F.sigmoid(agent.forward(batch_obs, args.model.split('_')[1], 'lr'))
    probs = probs*args.alpha + (1-args.alpha) * (1-probs)
    distr = Bernoulli(probs)
...
def train(epoch):
    ...
    for batch_obs, inputs_map, policy_sample, old_log_probs, targets, match, reward_sample, reward_map in rollout():
        ...
        for _ in range(args.ppo_epochs):
            ...
            surr2 = torch.clamp(ratios, 1.0 - args.clip_eps, 1.0 + args.clip_eps) * advantage
